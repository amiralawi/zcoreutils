...
pub fn main() !void {
    ...
    if (filenames.items.len == 1) {
        ...
        const src = filenames.items[0];
        const dest = filenames.items[1];
    ...
    const nfiles: usize = filenames.items.len - 1;
    const dest_dirname = filenames.items[nfiles];

    var dest_dir = try cwd.openDir(dest_dirname, .{});
    defer dest_dir.close();

    for (filenames.items[0..nfiles]) |src| {
        const filename = std.fs.path.basename(src);
        var dest_parts = [2][]const u8{ dest_dirname, filename };
        const dest = try std.fs.path.join(heapalloc, &dest_parts);
