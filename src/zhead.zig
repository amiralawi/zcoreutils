...
pub fn head_file_bytes(file: std.fs.File, nbytes: usize) !void {
    ...
    while (!finished) {
        ...
        const bytes_remaining = nbytes - bytes_written;
...
pub fn extractIntWithSuffix(str: []const u8) !i64 {
    ...
    const base = std.fmt.parseInt(i64, str[0 .. str.len - suffix_len], 10) catch |err| {
        return err;
    };
    return base * mult;
}
...
pub fn test_option_validity_and_store(str: []const u8) !bool {
    ...
    switch (last_option) {
        .bytecount => {
            const count = try extractIntWithSuffix(str);
            if (count < 0) {
        ...
        .linecount => {
            const count = try std.fmt.parseInt(i32, str, 10);
            if (count < 0) {
                // TODO - handle negative
            } else {
                count_max = @intCast(count);
    ...
    if (std.mem.startsWith(u8, str, "-") and str.len > 1) {
        ...
        for (str[1..]) |ch| {
            switch (ch) {
                'c' => {
                    last_option = .bytecount;
                    behavior_count = .bytes;
                },
                'n' => {
                    last_option = .linecount;
                    behavior_count = .lines;
                },
                'q' => {
                    header_behavior = .never;
                },
        ...
        if (test_option_validity_and_store(arg)) |store| {
            ...
            switch (err) {
                error.InvalidCharacter => {
                    const invalid_type = if (behavior_count == .lines) "lines" else "bytes";
                    try stdout.print("{s}: invalid number of {s}: '{s}'\n", .{ exe_name, invalid_type, arg });
                },
                else => {
                    try stdout.print("{s}: error processing argument '{s}': {any}\n", .{ exe_name, arg, err });
                },
            }
            return EXIT_FAILURE;
        }
