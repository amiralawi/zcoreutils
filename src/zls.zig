...
fn get_directory_contents(pathname: []const u8, alloc: std.mem.Allocator) !*std.ArrayList([]const u8) {
    ...
    // while (try iter.next()) |entry| {
    //     const strcpy: []u8 = try alloc.alloc(u8, entry.name.len);
...
fn calc_print_columns(w_console: usize, dir_contents: *const std.ArrayList([]const u8)) usize {
    ...
    const namelen_max = calc_longest_string(dir_contents.items);
    const namelen_min = calc_shortest_string(dir_contents.items);
    ...
    const ncols_max: usize = (w_console - namelen_max - 2) / (namelen_min + 3);
    const ncols_min: usize = 1;
    ...
    const len_wrap: usize = if (process_spaces) 2 else 0;
    const len_colsep: usize = if (process_spaces) 1 else 2;

    const nitems = dir_contents.items.len;
    var ncols: usize = 1;
    for (0..ncols_max) |e| {
        const cols = ncols_max - e;
        var w_line_max: usize = 0;
        const nrows = nitems / cols + @intFromBool((nitems % cols) > 0);
        for (0..cols) |i| {
...
const fs_grid = struct {
    ...
    fn init(dir_contents: *const std.ArrayList([]const u8), ncols: usize, alloc: std.mem.Allocator) !fs_grid {
        ...
        const nrows = ret.nitems / ncols + @intFromBool((ret.nitems % ncols) > 0);
        ret.columns = try ret.alloc.alloc(fs_column, ncols);
        ret.rows = try ret.alloc.alloc(fs_row, nrows);

        // fill grid
        for (0..nrows) |row| {
            const rowcols: usize = if (row >= ret.nitems % nrows) ncols else ncols - 1;
        ...
        // extract column data
        for (0..ncols) |col| {
            const col_start = @min(col * nrows, dir_contents.items.len);
            const col_end = @min(col_start + nrows, dir_contents.items.len);

            const col_has_space = contains_str_with_space(dir_contents.items[col_start..col_end]);
            const col_max_width = calc_longest_string(dir_contents.items[col_start..col_end]);
        ...
        for (self.rows) |row| {
            for (0..row.items.len) |i| {
                ...
                if (i < row.items.len - 1) {
                    ...
                    const col_sep = if (self.columns[i + 1].has_space) "  " else "   ";
                    conprint("{s}", .{col_sep});
                }
            }
            conprint("\n", .{});
...
fn print_in_columns(ncols: usize, dir_contents: *const std.ArrayList([]const u8)) void {
    const nitems = dir_contents.items.len;
    const nrows = nitems / ncols + @intFromBool((nitems % ncols) > 0);
    ...
    const wrapchar_nospace = if (process_spaces) " " else "";
    const column_separator = if (process_spaces) " " else "  ";
    ...
    for (0..nrows) |row| {
        for (0..ncols) |col| {
            ...
            if (col < ncols - 1) {
                ...
                const str_pad_target = calc_longest_string(dir_contents.items[col_start..col_end]);
                print("{s}", .{wrapchar});
...
pub fn main() !void {
    ...
    const keep_all: bool = flagmap['a'] or flagmap['A'];
    const keep_all_extra: bool = flagmap['A'];
    _ = keep_all_extra;
    // get directory contents
    const dir_contents: *std.ArrayList([]const u8) = try get_directory_contents(dirname, allocator);
    ...
    const w_console: usize = 80;
    // TODO - get actual console width
    const ncols = calc_print_columns(w_console, &dir_contents_filt);
