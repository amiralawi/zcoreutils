...
pub fn main() !u8 {
    ...
    const exetime: i128 = std.time.nanoTimestamp();
    ...
    for (filenames) |filename| {
        ...
        if (file) |f| {
            ...
            switch (behavior_touchtime) {
                ...
                .accessTimeOnly => {
                    atime = exetime;
                    mtime = std.os.linux.UTIME.OMIT;
                },
                .modifyTimeOnly => {
                    atime = std.os.linux.UTIME.OMIT;
                    mtime = exetime;
                },
            }

            std.fs.File.updateTimes(f, atime, mtime) catch |err|{
